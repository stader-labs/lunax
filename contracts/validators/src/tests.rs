...
mod tests {
    ...
    use crate::state::{Config, VALIDATOR_META, VMeta};
    use cosmwasm_std::testing::{mock_dependencies, mock_env, mock_info, MockStorage, MockApi, MockQuerier, MOCK_CONTRACT_ADDR};
    use cosmwasm_std::{coins, from_binary, Addr, OwnedDeps, MessageInfo, Env, Response, Empty, Validator, Decimal, FullDelegation, Coin, Uint128, SubMsg, StakingMsg, DistributionMsg, Attribute};
    ...
    fn test_add_validator() {
        ...
        assert!(VALIDATOR_META.may_load(deps.as_mut().storage, &valid1).unwrap().is_none());
        ...
        let err = execute(deps.as_mut(), env.clone(), info.clone(),
                          ExecuteMsg::AddValidator { val_addr: Addr::unchecked("valid0004").clone() },
        ).unwrap_err();
    ...
    fn test_stake() {
        ...
        let err = execute(deps.as_mut(), env.clone(),
                          mock_info(&pools_addr.to_string(), &[Coin::new(1200, "utest"), Coin::new(1000, "othercoin")]),
                          ExecuteMsg::Stake { val_addr: valid1.clone() },
        ).unwrap_err();
        assert!(matches!(err, ContractError::MultipleFunds {}));

        let err = execute(deps.as_mut(), env.clone(), pools_info.clone(),
                          ExecuteMsg::Stake { val_addr: Addr::unchecked("valid0004").clone() },
        ).unwrap_err();
    ...
    fn test_redeem_rewards() {
        ...
        let pools_info = mock_info(&pools_addr.to_string(), &[Coin::new(1200, "utest")]);
        let res = execute(deps.as_mut(), env.clone(), pools_info.clone(),
                          ExecuteMsg::RedeemRewards { validators: vec![valid1.clone()] },
        ).unwrap();
        assert!(res.messages.is_empty());
        assert_eq!(res.attributes, [Attribute { key: "failed_validators".to_string(), value: "valid0001".to_string() }]);
        ...
        assert_eq!(res.messages.len(), 1);
        assert_eq!(res.messages[0], SubMsg::new(DistributionMsg::WithdrawDelegatorReward {
            validator: valid1.to_string()
        }));
