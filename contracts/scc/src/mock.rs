...
//
// use addresses::{Addr, CanonicalAddr};
// use binary::Binary;
// use coins::Coin;
// use deps::OwnedDeps;
// use errors::{RecoverPubkeyError, StdError, StdResult, SystemError, VerificationError};
// #[cfg(feature = "stargate")]
// use crate::ibc::{
...
// };
// use results::{ContractResult, Empty, SystemResult};
// use serde::{from_slice, to_binary};
// use storage::MemoryStorage;
// use timestamp::Timestamp;
// use traits::{Api, Querier, QuerierResult};
// use types::{BlockInfo, ContractInfo, Env, MessageInfo};
// use crate::Attribute;
use cosmwasm_std::{Coin, OwnedDeps, MemoryStorage, Api, StdResult, CanonicalAddr, StdError, Addr, VerificationError, RecoverPubkeyError, BlockInfo, Env, Timestamp, ContractInfo, MessageInfo, SystemResult, ContractResult, Binary, Empty, SystemError, Querier, CustomQuery, QueryRequest, QuerierResult, from_slice, WasmQuery, BankQuery, BalanceResponse, to_binary, AllBalanceResponse, Attribute, Validator, FullDelegation, StakingQuery, BondedDenomResponse, AllValidatorsResponse, ValidatorResponse, AllDelegationsResponse, Uint128};
use sic_base::msg::{QueryMsg as sic_query_msg, QueryMsg, GetTotalTokensResponse, GetCurrentUndelegationBatchIdResponse};
use sic_base::msg::QueryMsg::GetCurrentUndelegationBatchId;
use serde::de::DeserializeOwned;
use std::collections::HashMap;
...
#[derive(Clone, Default)]
pub struct SICQuerier {
    total_tokens: Uint128,
    current_undelegation_batch_id: u64,
}

impl SICQuerier {
    pub fn new(total_tokens: Uint128, current_undelegation_batch_id: u64) -> Self {
        SICQuerier {
            total_tokens,
    ...
    pub fn query(&self, request: &sic_query_msg) -> QuerierResult {
        let contract_result: ContractResult<Binary> = match request {
            QueryMsg::GetTotalTokens {} => {
                let sic_res = GetTotalTokensResponse {
                    total_tokens: Option::from(self.total_tokens)
                };
                to_binary(&sic_res).into()
            }
            QueryMsg::GetCurrentUndelegationBatchId {} => {
                let sic_res = GetCurrentUndelegationBatchIdResponse {
                    current_undelegation_batch_id: self.current_undelegation_batch_id
                };
                to_binary(&sic_res).into()
            }
            _ => {}
...
mod tests {
    ...
    use hex_literal::hex;
    use cosmwasm_std::{coin, from_binary, Decimal, Delegation};
    ...
    #[test]
    fn sic_querier_total_tokens() {
        let sic = SICQuerier::new(Uint128::new(100_u128), 1);

        let all = sic.query(&sic_query_msg::GetTotalTokens {}).unwrap().unwrap();
        let sic_res: GetTotalTokensResponse = from_binary(&all).unwrap();
        assert_eq!(sic_res.total_tokens, Uint128::new(100_u128));
    }
