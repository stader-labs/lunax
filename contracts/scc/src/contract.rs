#[cfg(not(feature = "library"))]
use cosmwasm_std::entry_point;
use cosmwasm_std::{
    to_binary, Addr, BankMsg, Binary, Coin, Decimal, Deps, DepsMut, Env, MessageInfo, Response,
    StdError, StdResult, Uint128, WasmMsg,
};

use crate::error::ContractError;
use crate::helpers::{
    get_sic_fulfillable_undelegated_funds, get_staked_amount, get_strategy_shares_per_token_ratio,
    get_strategy_split, validate_user_portfolio,
};
use crate::msg::{
    ExecuteMsg, GetAllStrategiesResponse, GetConfigResponse, GetSharesPerTokenRatioResponse,
    GetStateResponse, GetStrategiesListResponse, GetStrategyInfoResponse,
    GetUndelegationBatchInfoResponse, GetUserResponse, GetUserRewardInfo, InstantiateMsg,
    MigrateMsg, QueryMsg, StrategyInfoQuery, UpdateUserAirdropsRequest, UpdateUserRewardsRequest,
    UserRewardInfoQuery, UserStrategyQueryInfo,
};
use crate::state::{
    get_default_s_t_ratio, BatchUndelegationRecord, Config, Cw20TokenContractsInfo, State,
    StrategyInfo, UndelegationBatchStatus, UserRewardInfo, UserStrategyInfo, UserStrategyPortfolio,
    UserUndelegationRecord, CONFIG, CW20_TOKEN_CONTRACTS_REGISTRY, DEFAULT_PAGINATION_LIMIT,
    MAX_PAGINATION_LIMIT, STATE, STRATEGY_MAP, UNDELEGATION_BATCH_MAP, USER_REWARD_INFO_MAP,
};
use crate::user::{compute_user_airdrops_across_strategies, get_user_airdrops};
use cw2::set_contract_version;
use cw_storage_plus::U64Key;
use sic_base::msg::ExecuteMsg as sic_execute_msg;
use stader_utils::coin_utils::{
    decimal_division_in_256, decimal_multiplication_in_256, decimal_subtraction_in_256,
    decimal_summation_in_256, get_decimal_from_uint128, merge_coin_vector, merge_dec_coin_vector,
    uint128_from_decimal, CoinVecOp, DecCoin, DecCoinVecOp, Operation,
};
use stader_utils::helpers::{query_cw20_token_balance, send_funds_msg};
use std::collections::HashMap;

const CONTRACT_NAME: &str = "scc";
const CONTRACT_VERSION: &str = env!("CARGO_PKG_VERSION");

#[cfg_attr(not(feature = "library"), entry_point)]
pub fn instantiate(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    msg: InstantiateMsg,
) -> Result<Response, ContractError> {
    let state = State {
        manager: info.sender.clone(),
        delegator_contract: deps.api.addr_validate(msg.delegator_contract.as_str())?,
        scc_denom: "uluna".to_string(),
        contract_genesis_block_height: _env.block.height,
        contract_genesis_timestamp: _env.block.time,
        next_undelegation_id: 0,
        next_strategy_id: 1,
        rewards_in_scc: Uint128::zero(),
    };
    STATE.save(deps.storage, &state)?;

    // create strategy_id 0 which is "retain_rewards". It's a special strategy where rewards
    // of the user just sit in SCC
    STRATEGY_MAP.save(
        deps.storage,
        U64Key::new(0),
        &StrategyInfo::default("retain_rewards".to_string()),
    )?;

    let config = Config {
        default_user_portfolio: vec![],
        fallback_strategy: 0,
        max_undelegation_limit: msg.max_undelegation_limit.unwrap_or(20),
    };
    CONFIG.save(deps.storage, &config)?;

    set_contract_version(deps.storage, CONTRACT_NAME, CONTRACT_VERSION)?;

    Ok(Response::new()
        .add_attribute("method", "instantiate")
        .add_attribute("owner", info.sender))
}

#[cfg_attr(not(feature = "library"), entry_point)]
pub fn migrate(_deps: DepsMut, _env: Env, _msg: MigrateMsg) -> Result<Response, ContractError> {
    Ok(Response::default())
}

#[cfg_attr(not(feature = "library"), entry_point)]
pub fn execute(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    msg: ExecuteMsg,
) -> Result<Response, ContractError> {
    match msg {
        ExecuteMsg::RegisterStrategy {
            strategy_name,
            unbonding_period,
            unbonding_buffer,
            sic_contract_address,
            undelegation_frequency,
        } => register_strategy(
            deps,
            _env,
            info,
            strategy_name,
            sic_contract_address,
            unbonding_period,
            unbonding_buffer,
            undelegation_frequency,
        ),
        ExecuteMsg::UpdateStrategy {
            strategy_id,
            unbonding_period,
            unbonding_buffer,
            sic_contract_address,
            undelegation_frequency,
            is_active,
        } => update_strategy(
            deps,
            _env,
            info,
            strategy_id,
            unbonding_period,
            unbonding_buffer,
            sic_contract_address,
            undelegation_frequency,
            is_active,
        ),
        ExecuteMsg::UpdateUserPortfolio { user_portfolio } => {
            update_user_portfolio(deps, _env, info, user_portfolio)
        }
        ExecuteMsg::UpdateUserRewards {
            update_user_rewards_requests,
        } => update_user_rewards(deps, _env, info, update_user_rewards_requests),
        ExecuteMsg::UpdateUserAirdrops {
            update_user_airdrops_requests,
        } => update_user_airdrops(deps, _env, info, update_user_airdrops_requests),
        ExecuteMsg::UndelegateRewards {
            amount,
            strategy_id,
        } => undelegate_user_rewards(deps, _env, info, amount, strategy_id),
        ExecuteMsg::ClaimAirdrops {
            strategy_id,
            amount,
            airdrop_token: denom,
            stage,
            proof,
        } => claim_airdrops(deps, _env, info, strategy_id, denom, amount, stage, proof),
        ExecuteMsg::UpdateAirdropPointers {
            strategy_id,
            airdrop_token,
        } => update_airdrop_pointers(deps, _env, info, strategy_id, airdrop_token),
        ExecuteMsg::WithdrawRewards {
            undelegation_id,
            strategy_id,
        } => withdraw_rewards(deps, _env, info, undelegation_id, strategy_id),
        ExecuteMsg::WithdrawAirdrops {} => withdraw_airdrops(deps, _env, info),
        ExecuteMsg::RegisterCw20Contracts {
            denom,
            cw20_contract,
            airdrop_contract,
        } => {
            update_cw20_contracts_registry(deps, _env, info, denom, cw20_contract, airdrop_contract)
        }
        ExecuteMsg::UndelegateFromStrategy { strategy_id } => {
            undelegate_from_strategies(deps, _env, info, strategy_id)
        }
        ExecuteMsg::FetchUndelegatedRewardsFromStrategy {
            strategy_id,
            pagination_count,
        } => {
            fetch_undelegated_rewards_from_strategy(deps, _env, info, strategy_id, pagination_count)
        }
        ExecuteMsg::DepositFunds { strategy_override } => {
            deposit_funds(deps, _env, info, strategy_override)
        }
        ExecuteMsg::UpdateConfig {
            delegator_contract,
            default_user_portfolio,
            fallback_strategy,
            max_undelegation_limit,
        } => update_config(
            deps,
            _env,
            info,
            delegator_contract,
            default_user_portfolio,
            fallback_strategy,
            max_undelegation_limit,
        ),
    }
}

pub fn update_config(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    delegator_contract: Option<String>,
    default_user_portfolio: Option<Vec<UserStrategyPortfolio>>,
    fallback_strategy: Option<u64>,
    max_undelegation_limit: Option<u64>,
) -> Result<Response, ContractError> {
    let mut state = STATE.load(deps.storage)?;
    let mut config = CONFIG.load(deps.storage)?;
    if info.sender != state.manager {
        return Err(ContractError::Unauthorized {});
    }

    if let Some(dc) = delegator_contract {
        state.delegator_contract = deps.api.addr_validate(dc.as_str())?;
    }

    if let Some(fs) = fallback_strategy {
        // check if strategy exists
        if STRATEGY_MAP
            .may_load(deps.storage, U64Key::new(fs))?
            .is_none()
        {
            return Err(ContractError::StrategyInfoDoesNotExist {});
        }

        config.fallback_strategy = fs;
    }

    if let Some(dup) = default_user_portfolio {
        if validate_user_portfolio(deps.storage, &dup).is_err() {
            return Err(ContractError::InvalidUserPortfolio {});
        }

        config.default_user_portfolio = dup;
    }

    if let Some(mul) = max_undelegation_limit {
        config.max_undelegation_limit = mul;
    }

    STATE.save(deps.storage, &state)?;
    CONFIG.save(deps.storage, &config)?;

    Ok(Response::default())
}

#[allow(clippy::too_many_arguments)]
pub fn update_strategy(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    strategy_id: u64,
    unbonding_period: Option<u64>,
    unbonding_buffer: Option<u64>,
    sic_contract_address: Option<String>,
    undelegation_frequency: Option<u64>,
    is_active: Option<bool>,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;
    if info.sender != state.manager {
        return Err(ContractError::Unauthorized {});
    }

    let mut strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::from(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };

    strategy_info.unbonding_period = unbonding_period.unwrap_or(strategy_info.unbonding_period);
    strategy_info.unbonding_buffer = unbonding_buffer.unwrap_or(strategy_info.unbonding_buffer);
    strategy_info.undelegation_cooldown =
        undelegation_frequency.unwrap_or(strategy_info.undelegation_cooldown);
    strategy_info.is_active = is_active.unwrap_or(strategy_info.is_active);
    strategy_info.sic_contract_address = if let Some(sic) = sic_contract_address {
        deps.api.addr_validate(sic.as_str())?
    } else {
        strategy_info.sic_contract_address
    };

    STRATEGY_MAP.save(deps.storage, U64Key::from(strategy_id), &strategy_info)?;

    Ok(Response::default())
}

pub fn fetch_undelegated_rewards_from_strategy(
    deps: DepsMut,
    env: Env,
    _info: MessageInfo,
    strategy_id: u64,
    pagination_count: Option<u64>,
) -> Result<Response, ContractError> {
    let mut messages: Vec<WasmMsg> = vec![];
    let mut failed_undelegation_batches: Vec<String> = vec![];
    let mut undelegation_batches_released: Vec<String> = vec![];
    let mut total_amount_to_fetch: Uint128 = Uint128::zero();
    let mut strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };

    let last_reconciled_batch_id = strategy_info.next_reconciliation_batch_id;
    let mut current_batch_being_reconciled = last_reconciled_batch_id;
    let limit = pagination_count
        .unwrap_or(DEFAULT_PAGINATION_LIMIT as u64)
        .min(MAX_PAGINATION_LIMIT as u64);
    let limit = std::cmp::min(
        last_reconciled_batch_id + limit,
        strategy_info.next_undelegation_batch_id,
    );
    for i in (last_reconciled_batch_id)..limit {
        let mut undelegation_batch = if let Some(undelegation_batch) = UNDELEGATION_BATCH_MAP
            .may_load(deps.storage, (U64Key::new(i), U64Key::new(strategy_id)))?
        {
            undelegation_batch
        } else {
            // move the pointer if there is no undelegation batch has been found.
            failed_undelegation_batches.push(format!(
                "{}:{}",
                strategy_id.to_string(),
                i.to_string(),
            ));
            current_batch_being_reconciled += 1;
            continue;
        };

        // undelegation batch has been checked for slashing. we can move the pointer, if the
        // batch has been slashed
        if matches!(
            undelegation_batch.undelegation_batch_status,
            UndelegationBatchStatus::Done
        ) {
            undelegation_batches_released.push(format!(
                "{}:{}",
                strategy_id.to_string(),
                i.to_string(),
            ));
            current_batch_being_reconciled += 1;
            continue;
        }

        // undelegation batch is still in unbonding period
        // error out when we encounter a batch still in undelegation. we need to wait for it and not pass by it
        if undelegation_batch.est_release_time.is_none()
            || undelegation_batch
                .est_release_time
                .unwrap()
                .gt(&env.block.time)
        {
            return Err(ContractError::UndelegationBatchInUnbondingPeriod(i));
        }

        let undelegation_batch_amount = undelegation_batch.amount;

        // query the undelegated funds which we will receive from the SIC on best effort
        // TODO: bchain99 - this assignment works in the assumption that undelegation slashing is rare.
        // if we don't get back any response from the SIC, we can assume the full amount.
        // if there was indeed some slashing, we would have to compensate for it.
        let mut fulfillable_amount: Uint128 = undelegation_batch_amount;
        if let Ok(result) = get_sic_fulfillable_undelegated_funds(
            deps.querier,
            undelegation_batch_amount,
            &strategy_info.sic_contract_address,
        ) {
            fulfillable_amount = result
                .undelegated_funds
                .unwrap_or(undelegation_batch_amount)
        }

        let unbonding_slashing_ratio = if fulfillable_amount.lt(&undelegation_batch_amount) {
            Decimal::from_ratio(fulfillable_amount, undelegation_batch_amount)
        } else {
            Decimal::one()
        };

        undelegation_batch.unbonding_slashing_ratio = unbonding_slashing_ratio;
        undelegation_batch.undelegation_batch_status = UndelegationBatchStatus::Done;

        total_amount_to_fetch = total_amount_to_fetch
            .checked_add(undelegation_batch_amount)
            .unwrap();
        UNDELEGATION_BATCH_MAP.save(
            deps.storage,
            (U64Key::new(i), U64Key::new(strategy_id)),
            &undelegation_batch,
        )?;

        current_batch_being_reconciled += 1;
    }

    strategy_info.next_reconciliation_batch_id = current_batch_being_reconciled;
    STRATEGY_MAP.save(deps.storage, U64Key::new(strategy_id), &strategy_info)?;

    if !total_amount_to_fetch.is_zero() {
        messages.push(WasmMsg::Execute {
            contract_addr: strategy_info.sic_contract_address.to_string(),
            msg: to_binary(&sic_execute_msg::TransferUndelegatedRewards {
                amount: total_amount_to_fetch,
            })
            .unwrap(),
            funds: vec![],
        });
    }

    Ok(Response::new()
        .add_messages(messages)
        .add_attribute(
            "failed_undelegation_batches",
            failed_undelegation_batches.join(","),
        )
        .add_attribute(
            "undelegation_batches_released",
            undelegation_batches_released.join(","),
        ))
}

pub fn undelegate_from_strategies(
    deps: DepsMut,
    env: Env,
    _info: MessageInfo,
    strategy_id: u64,
) -> Result<Response, ContractError> {
    let mut messages: Vec<WasmMsg> = vec![];
    let mut strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };

    let strategy_undelegation_shares: Decimal = strategy_info.current_undelegated_shares;
    if strategy_undelegation_shares.is_zero() {
        return Err(ContractError::NoPendingUndelegations {});
    }

    // check the cooldown period
    if env
        .block
        .time
        .minus_seconds(strategy_info.last_undelegated_time.seconds())
        .seconds()
        .lt(&strategy_info.undelegation_cooldown)
    {
        return Err(ContractError::UndelegationInCooldown {});
    }

    let strategy_s_t_ratio = match get_strategy_shares_per_token_ratio(deps.querier, &strategy_info)
    {
        Ok(result) => result,
        Err(_) => return Err(ContractError::SICFailedToReturnResult {}),
    };

    let undelegation_amount = uint128_from_decimal(decimal_division_in_256(
        strategy_undelegation_shares,
        strategy_s_t_ratio,
    ));

    let current_undelegation_batch_id = strategy_info.next_undelegation_batch_id;

    UNDELEGATION_BATCH_MAP.update(
        deps.storage,
        (
            U64Key::new(current_undelegation_batch_id),
            U64Key::new(strategy_id),
        ),
        |batch_opt| -> Result<_, ContractError> {
            if batch_opt.is_none() {
                panic!(
                    "undelegation batch id {:?} not found",
                    current_undelegation_batch_id
                );
            }

            let mut batch = batch_opt.unwrap();
            batch.amount = undelegation_amount;
            batch.shares = strategy_undelegation_shares;
            batch.undelegation_s_t_ratio = strategy_s_t_ratio;
            batch.est_release_time =
                Some(env.block.time.plus_seconds(strategy_info.unbonding_period));
            batch.withdrawal_time = Some(
                env.block
                    .time
                    .plus_seconds(strategy_info.unbonding_period + strategy_info.unbonding_buffer),
            );
            batch.undelegation_batch_status = UndelegationBatchStatus::InProgress;
            Ok(batch)
        },
    )?;

    strategy_info.total_shares = decimal_subtraction_in_256(
        strategy_info.total_shares,
        strategy_info.current_undelegated_shares,
    );
    strategy_info.current_undelegated_shares = Decimal::zero();
    strategy_info.next_undelegation_batch_id = current_undelegation_batch_id + 1;
    strategy_info.last_undelegated_time = env.block.time;

    messages.push(WasmMsg::Execute {
        contract_addr: String::from(strategy_info.sic_contract_address.clone()),
        msg: to_binary(&sic_execute_msg::UndelegateRewards {
            amount: undelegation_amount,
        })
        .unwrap(),
        funds: vec![],
    });

    STRATEGY_MAP.save(deps.storage, U64Key::new(strategy_id), &strategy_info)?;

    Ok(Response::new().add_messages(messages))
}

pub fn update_cw20_contracts_registry(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    denom: String,
    cw20_token_contract: String,
    airdrop_contract: String,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;
    if info.sender != state.manager {
        return Err(ContractError::Unauthorized {});
    }

    CW20_TOKEN_CONTRACTS_REGISTRY.save(
        deps.storage,
        denom.to_lowercase(),
        &Cw20TokenContractsInfo {
            airdrop_contract: deps.api.addr_validate(airdrop_contract.as_str())?,
            cw20_token_contract: deps.api.addr_validate(cw20_token_contract.as_str())?,
        },
    )?;

    Ok(Response::default())
}

pub fn withdraw_pending_rewards(
    deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
    user_addr: Addr,
    amount: Uint128,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;

    let mut user_reward_info =
        if let Some(user_reward_info) = USER_REWARD_INFO_MAP.may_load(deps.storage, &user_addr)? {
            user_reward_info
        } else {
            return Err(ContractError::UserRewardInfoDoesNotExist {});
        };

    if user_reward_info.pending_rewards.lt(&amount) || user_reward_info.pending_rewards.is_zero() {
        return Err(ContractError::UserDoesNotHaveEnoughRewards {});
    }

    user_reward_info.pending_rewards = user_reward_info
        .pending_rewards
        .checked_sub(amount)
        .unwrap();

    USER_REWARD_INFO_MAP.save(deps.storage, &user_addr, &user_reward_info)?;

    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.rewards_in_scc = state.rewards_in_scc.checked_sub(amount).unwrap();
        Ok(state)
    })?;

    Ok(Response::new().add_message(send_funds_msg(
        &user_addr,
        &[Coin::new(amount.u128(), state.scc_denom)],
    )))
}

pub fn undelegate_user_rewards(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    amount: Uint128,
    strategy_id: u64,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;

    let user_addr = info.sender.clone();

    if strategy_id.eq(&0) {
        return withdraw_pending_rewards(deps, _env, info, user_addr, amount);
    }

    if amount.is_zero() {
        return Err(ContractError::CannotUndelegateZeroFunds {});
    }

    let mut strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };

    let mut user_reward_info =
        if let Some(user_reward_info) = USER_REWARD_INFO_MAP.may_load(deps.storage, &user_addr)? {
            user_reward_info
        } else {
            return Err(ContractError::UserRewardInfoDoesNotExist {});
        };

    // check if the undelegation amount crosses the user total undelegatable amount
    let user_strategy_info = if let Some(user_strategy_info) = user_reward_info
        .strategies
        .iter_mut()
        .find(|x| x.strategy_id.eq(&strategy_id))
    {
        user_strategy_info
    } else {
        return Err(ContractError::UserNotInStrategy {});
    };

    let config = CONFIG.load(deps.storage)?;
    if user_reward_info
        .undelegation_records
        .len()
        .ge(&(config.max_undelegation_limit as usize))
    {
        return Err(ContractError::UserUndelegationRecordLimitExceeded {});
    }

    // create the undelegation batch if it doesn't exist and keep it in pending status
    UNDELEGATION_BATCH_MAP.update(
        deps.storage,
        (
            U64Key::new(strategy_info.next_undelegation_batch_id),
            U64Key::new(strategy_id),
        ),
        |batch_opt| -> Result<_, ContractError> {
            if batch_opt.is_none() {
                return Ok(BatchUndelegationRecord {
                    amount: Uint128::zero(),
                    shares: Decimal::zero(),
                    unbonding_slashing_ratio: Decimal::one(),
                    undelegation_s_t_ratio: get_default_s_t_ratio(),
                    create_time: None,
                    // est_release_time will be filled up in the undelegate_from_strategies call
                    est_release_time: None,
                    withdrawal_time: None,
                    undelegation_batch_status: UndelegationBatchStatus::Pending,
                });
            }

            Ok(batch_opt.unwrap())
        },
    )?;

    // update the user airdrop pointer and allocate the user pending airdrops for the strategy
    let user_airdrops = get_user_airdrops(
        &strategy_info.global_airdrop_pointer,
        &user_strategy_info.airdrop_pointer,
        user_strategy_info.shares,
    )
    .unwrap_or_default();
    user_strategy_info.airdrop_pointer = strategy_info.global_airdrop_pointer.clone();

    user_reward_info.pending_airdrops = merge_coin_vector(
        &user_reward_info.pending_airdrops,
        CoinVecOp {
            fund: user_airdrops,
            operation: Operation::Add,
        },
    );

    // subtract the user shares based on how much they want to withdraw.
    let strategy_shares_per_token_ratio =
        match get_strategy_shares_per_token_ratio(deps.querier, &strategy_info) {
            Ok(result) => result,
            Err(_) => {
                return Err(ContractError::SICFailedToReturnResult {});
            }
        };

    let user_total_shares = user_strategy_info.shares;
    let user_undelegated_shares = decimal_multiplication_in_256(
        strategy_shares_per_token_ratio,
        Decimal::from_ratio(amount, 1_u128),
    );

    if user_undelegated_shares.gt(&user_total_shares) {
        return Err(ContractError::UserDoesNotHaveEnoughRewards {});
    }

    user_strategy_info.shares =
        decimal_subtraction_in_256(user_total_shares, user_undelegated_shares);
    strategy_info.current_undelegated_shares = decimal_summation_in_256(
        strategy_info.current_undelegated_shares,
        user_undelegated_shares,
    );

    user_reward_info
        .undelegation_records
        .push(UserUndelegationRecord {
            // there may multiple records with the same id. when withdrawing, use amount as the tie-breaker in such a case.
            id: state.next_undelegation_id,
            amount,
            shares: user_undelegated_shares,
            strategy_id,
            undelegation_batch_id: strategy_info.next_undelegation_batch_id,
        });

    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.next_undelegation_id += 1;
        Ok(state)
    })?;

    STRATEGY_MAP.save(deps.storage, U64Key::new(strategy_id), &strategy_info)?;
    USER_REWARD_INFO_MAP.save(deps.storage, &user_addr, &user_reward_info)?;

    Ok(Response::default())
}

pub fn update_airdrop_pointers(
    deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
    strategy_id: u64,
    airdrop_token: String,
) -> Result<Response, ContractError> {
    let cw20_token_contracts = if let Some(cw20_token_contracts) =
        CW20_TOKEN_CONTRACTS_REGISTRY.may_load(deps.storage, airdrop_token.to_lowercase())?
    {
        cw20_token_contracts
    } else {
        return Err(ContractError::AirdropNotRegistered {});
    };

    let mut strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };
    let sic_address = strategy_info.sic_contract_address.clone();

    let sic_airdrop_balance = query_cw20_token_balance(
        deps.querier,
        cw20_token_contracts.cw20_token_contract.clone(),
        sic_address.to_string(),
    )?;
    if sic_airdrop_balance.is_zero() {
        return Err(ContractError::ZeroAmount {});
    }

    let effective_shares = decimal_subtraction_in_256(
        strategy_info.total_shares,
        strategy_info.current_undelegated_shares,
    );
    let airdrop_coin = Coin::new(sic_airdrop_balance.u128(), airdrop_token.clone());

    strategy_info.total_airdrops_accumulated = merge_coin_vector(
        &strategy_info.total_airdrops_accumulated,
        CoinVecOp {
            fund: vec![airdrop_coin],
            operation: Operation::Add,
        },
    );

    strategy_info.global_airdrop_pointer = merge_dec_coin_vector(
        &strategy_info.global_airdrop_pointer,
        DecCoinVecOp {
            fund: vec![DecCoin {
                amount: decimal_division_in_256(
                    Decimal::from_ratio(sic_airdrop_balance, 1_u128),
                    effective_shares,
                ),
                denom: airdrop_token.clone(),
            }],
            operation: Operation::Add,
        },
    );

    STRATEGY_MAP.save(deps.storage, U64Key::new(strategy_id), &strategy_info)?;

    Ok(Response::new().add_message(WasmMsg::Execute {
        contract_addr: strategy_info.sic_contract_address.to_string(),
        msg: to_binary(&sic_execute_msg::TransferAirdropsToScc {
            cw20_token_contract: cw20_token_contracts.cw20_token_contract.to_string(),
            airdrop_token,
            amount: sic_airdrop_balance,
        })?,
        funds: vec![],
    }))
}

#[allow(clippy::too_many_arguments)]
pub fn claim_airdrops(
    deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
    strategy_id: u64,
    denom: String,
    amount: Uint128,
    stage: u8,
    proof: Vec<String>,
) -> Result<Response, ContractError> {
    if amount.is_zero() {
        return Err(ContractError::ZeroAmount {});
    }

    let cw20_token_contracts = if let Some(cw20_token_contracts) =
        CW20_TOKEN_CONTRACTS_REGISTRY.may_load(deps.storage, denom.to_lowercase())?
    {
        cw20_token_contracts
    } else {
        return Err(ContractError::AirdropNotRegistered {});
    };

    let strategy_info = if let Some(strategy_info) =
        STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
    {
        strategy_info
    } else {
        return Err(ContractError::StrategyInfoDoesNotExist {});
    };
    let sic_address = strategy_info.sic_contract_address;

    Ok(Response::new().add_message(WasmMsg::Execute {
        contract_addr: String::from(sic_address),
        msg: to_binary(&sic_execute_msg::ClaimAirdrops {
            airdrop_token_contract: cw20_token_contracts.airdrop_contract.to_string(),
            airdrop_token: denom,
            amount,
            stage,
            proof,
        })
        .unwrap(),
        funds: vec![],
    }))
}

pub fn withdraw_airdrops(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
) -> Result<Response, ContractError> {
    // transfer the airdrops in pending_airdrops vector in user_reward_info to the user
    let user_addr = info.sender;

    let mut user_reward_info =
        if let Some(user_reward_info) = USER_REWARD_INFO_MAP.may_load(deps.storage, &user_addr)? {
            user_reward_info
        } else {
            return Err(ContractError::UserRewardInfoDoesNotExist {});
        };

    compute_user_airdrops_across_strategies(deps.storage, &mut user_reward_info);

    let mut messages: Vec<WasmMsg> = vec![];
    // iterate thru all airdrops and transfer ownership to them to the user
    user_reward_info
        .pending_airdrops
        .iter_mut()
        .for_each(|user_airdrop| {
            let airdrop_denom = &user_airdrop.denom.to_lowercase();
            let airdrop_amount = user_airdrop.amount;

            let cw20_token_contracts = CW20_TOKEN_CONTRACTS_REGISTRY
                .may_load(deps.storage, airdrop_denom.to_string())
                .unwrap();

            if cw20_token_contracts.is_none() || airdrop_amount.is_zero() {
                return;
            }

            messages.push(WasmMsg::Execute {
                contract_addr: String::from(cw20_token_contracts.unwrap().cw20_token_contract),
                msg: to_binary(&cw20::Cw20ExecuteMsg::Transfer {
                    recipient: user_addr.to_string(),
                    amount: airdrop_amount,
                })
                .unwrap(),
                funds: vec![],
            });

            // the airdrop is completely transferred back to the user
            user_airdrop.amount = Uint128::zero();
        });

    USER_REWARD_INFO_MAP.save(deps.storage, &user_addr, &user_reward_info)?;

    Ok(Response::new().add_messages(messages))
}

pub fn withdraw_rewards(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    undelegation_id: u64,
    strategy_id: u64,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;
    let user_addr = info.sender;

    let mut user_reward_info =
        if let Some(user_reward_info) = USER_REWARD_INFO_MAP.may_load(deps.storage, &user_addr)? {
            user_reward_info
        } else {
            return Err(ContractError::UserRewardInfoDoesNotExist {});
        };

    let user_undelegation_record: &UserUndelegationRecord;
    let undelegation_record_index = if let Some(i) =
        (0..user_reward_info.undelegation_records.len()).find(|&i| {
            user_reward_info.undelegation_records[i]
                .id
                .eq(&undelegation_id)
                && user_reward_info.undelegation_records[i]
                    .strategy_id
                    .eq(&strategy_id)
        }) {
        user_undelegation_record = &user_reward_info.undelegation_records[i];
        i
    } else {
        return Err(ContractError::UndelegationRecordNotFound {});
    };

    let undelegation_batch_id = U64Key::new(user_undelegation_record.undelegation_batch_id);
    let undelegation_batch = if let Some(undelegation_batch) = UNDELEGATION_BATCH_MAP.may_load(
        deps.storage,
        (undelegation_batch_id, U64Key::new(strategy_id)),
    )? {
        undelegation_batch
    } else {
        return Err(ContractError::UndelegationBatchNotFound {});
    };

    if undelegation_batch.withdrawal_time.is_none()
        || undelegation_batch
            .withdrawal_time
            .unwrap()
            .gt(&_env.block.time)
    {
        return Err(ContractError::UndelegationInUnbondingPeriod {});
    }

    // undelegation batch has not been released yet
    if !matches!(
        undelegation_batch.undelegation_batch_status,
        UndelegationBatchStatus::Done
    ) {
        return Err(ContractError::UndelegationBatchNotReleased {});
    }

    let withdrawable_amount = uint128_from_decimal(decimal_division_in_256(
        user_undelegation_record.shares,
        undelegation_batch.undelegation_s_t_ratio,
    ));
    // now apply unbonding slashing to the withdrawable amount
    let effective_withdrawable_amount = uint128_from_decimal(decimal_multiplication_in_256(
        Decimal::from_ratio(withdrawable_amount, 1_u128),
        undelegation_batch.unbonding_slashing_ratio,
    ));

    // remove undelegation record
    user_reward_info
        .undelegation_records
        .remove(undelegation_record_index);

    USER_REWARD_INFO_MAP.save(deps.storage, &user_addr, &user_reward_info)?;

    Ok(Response::new().add_message(BankMsg::Send {
        to_address: String::from(user_addr),
        amount: vec![Coin::new(
            effective_withdrawable_amount.u128(),
            state.scc_denom,
        )],
    }))
}

pub fn register_strategy(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    strategy_name: String,
    sic_contract_address: String,
    unbonding_period: u64,
    unbonding_buffer: u64,
    undelegation_frequency: u64,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;
    if info.sender != state.manager {
        return Err(ContractError::Unauthorized {});
    }

    let sic_contract_address = deps.api.addr_validate(sic_contract_address.as_str())?;
    let strategy_id: u64 = state.next_strategy_id;

    STRATEGY_MAP.save(
        deps.storage,
        U64Key::new(strategy_id),
        &StrategyInfo::new(
            strategy_name.to_lowercase(),
            sic_contract_address,
            unbonding_period,
            unbonding_buffer,
            undelegation_frequency,
        ),
    )?;

    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.next_strategy_id += 1;
        Ok(state)
    })?;

    Ok(Response::default())
}

pub fn update_user_portfolio(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    user_portfolio: Vec<UserStrategyPortfolio>,
) -> Result<Response, ContractError> {
    let user_addr = info.sender;

    // check if the entire portfolio deposit fraction is less than 1. else abort the tx
    // do bunch of sanity checks
    if validate_user_portfolio(deps.storage, &user_portfolio).is_err() {
        return Err(ContractError::InvalidUserPortfolio {});
    }

    USER_REWARD_INFO_MAP.update(
        deps.storage,
        &user_addr,
        |reward_info_opt| -> Result<_, ContractError> {
            let mut user_reward_info = reward_info_opt.unwrap_or_else(UserRewardInfo::default);
            user_reward_info.user_portfolio = user_portfolio;
            Ok(user_reward_info)
        },
    )?;

    Ok(Response::default())
}

pub fn deposit_funds(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    strategy_override: Option<u64>,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;

    // auth checks
    if info.funds.is_empty() {
        return Err(ContractError::NoFundsSent {});
    }

    if info.funds.len() > 1 {
        return Err(ContractError::MultipleCoinsSent {});
    }

    let funds = info.funds[0].clone();
    if state.scc_denom != funds.denom {
        return Err(ContractError::WrongDenomSent {});
    }

    if funds.amount.is_zero() {
        return Ok(Response::new().add_attribute("0 funds sent", "1"));
    }

    let user_addr = info.sender.clone();

    try_deposit_funds_to_strategies(
        deps,
        _env,
        info,
        vec![UpdateUserRewardsRequest {
            user: user_addr,
            funds: funds.amount,
            strategy_id: strategy_override,
        }],
    )
}

pub fn update_user_rewards(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    update_user_rewards_requests: Vec<UpdateUserRewardsRequest>,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;
    if info.sender != state.delegator_contract {
        return Err(ContractError::Unauthorized {});
    }

    try_deposit_funds_to_strategies(deps, _env, info, update_user_rewards_requests)
}

fn try_deposit_funds_to_strategies(
    deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
    update_user_rewards_requests: Vec<UpdateUserRewardsRequest>,
) -> Result<Response, ContractError> {
    let state = STATE.load(deps.storage)?;

    if update_user_rewards_requests.is_empty() {
        return Ok(Response::new().add_attribute("zero_update_user_rewards_requests", "1"));
    }

    let mut failed_strategies: Vec<String> = vec![];
    let mut users_with_zero_deposits: Vec<String> = vec![];
    // cache for the S/T ratio per strategy. We fetch it once and cache it up.
    let mut strategy_to_s_t_ratio: HashMap<u64, Decimal> = HashMap::new();
    let mut strategy_to_funds: HashMap<Addr, Uint128> = HashMap::new();
    let mut total_rewards_in_scc: Uint128 = Uint128::zero();
    let mut messages: Vec<WasmMsg> = vec![];
    for update_user_rewards_request in update_user_rewards_requests {
        let user_addr = update_user_rewards_request.user;
        let funds = update_user_rewards_request.funds;
        let strategy_override = update_user_rewards_request.strategy_id;

        if funds.is_zero() {
            users_with_zero_deposits.push(user_addr.to_string());
            continue;
        }

        let config = CONFIG.load(deps.storage)?;
        let mut user_reward_info = USER_REWARD_INFO_MAP
            .may_load(deps.storage, &user_addr)?
            .unwrap_or_else(|| UserRewardInfo::new(config.default_user_portfolio.clone()));

        let strategy_split = get_strategy_split(
            deps.storage,
            &config,
            strategy_override,
            &user_reward_info,
            funds,
        )?;

        for s2a in strategy_split.iter() {
            let strategy_id = *s2a.0;
            let amount = *s2a.1;

            if amount.is_zero() {
                continue;
            }

            // this is the retain rewards strategy
            if strategy_id.eq(&0) {
                user_reward_info.pending_rewards = user_reward_info
                    .pending_rewards
                    .checked_add(amount)
                    .unwrap();
                total_rewards_in_scc = total_rewards_in_scc.checked_add(amount).unwrap();
                continue;
            }

            let mut strategy_info = if let Some(strategy_info) =
                STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
            {
                strategy_info
            } else {
                // this ideally won't happen as non-existing strategies funds will be pushed
                // to the fall back strategy
                failed_strategies.push(strategy_id.to_string());
                continue;
            };

            // fetch the total tokens from the SIC contract and update the S/T ratio for the strategy
            // compute the S/T ratio only once as we are batching up reward transfer messages. Jus' cache
            // it up in a map like we always do.
            let current_strategy_shares_per_token_ratio: Decimal;
            if !strategy_to_s_t_ratio.contains_key(&strategy_id) {
                match get_strategy_shares_per_token_ratio(deps.querier, &strategy_info) {
                    Ok(result) => {
                        current_strategy_shares_per_token_ratio = result;
                    }
                    Err(_) => return Err(ContractError::SICFailedToReturnResult {}),
                }
                strategy_to_s_t_ratio.insert(strategy_id, current_strategy_shares_per_token_ratio);
            } else {
                current_strategy_shares_per_token_ratio =
                    *strategy_to_s_t_ratio.get(&strategy_id).unwrap();
            }

            let mut user_strategy_info = if let Some(i) = (0..user_reward_info.strategies.len())
                .find(|&i| user_reward_info.strategies[i].strategy_id.eq(&strategy_id))
            {
                &mut user_reward_info.strategies[i]
            } else {
                let new_user_strategy_info: UserStrategyInfo = UserStrategyInfo::new(
                    strategy_id,
                    strategy_info.global_airdrop_pointer.clone(),
                );
                user_reward_info.strategies.push(new_user_strategy_info);
                user_reward_info.strategies.last_mut().unwrap()
            };

            // update the user airdrop pointer and allocate the user pending airdrops for the strategy
            let user_airdrops = get_user_airdrops(
                &strategy_info.global_airdrop_pointer,
                &user_strategy_info.airdrop_pointer,
                user_strategy_info.shares,
            )
            .unwrap_or_default();
            user_strategy_info.airdrop_pointer = strategy_info.global_airdrop_pointer.clone();
            user_reward_info.pending_airdrops = merge_coin_vector(
                &user_reward_info.pending_airdrops,
                CoinVecOp {
                    fund: user_airdrops,
                    operation: Operation::Add,
                },
            );

            // update user shares based on the S/T ratio
            let user_shares = decimal_multiplication_in_256(
                current_strategy_shares_per_token_ratio,
                get_decimal_from_uint128(amount),
            );
            user_strategy_info.shares =
                decimal_summation_in_256(user_strategy_info.shares, user_shares);
            // update total strategy shares by adding up the user_shares
            strategy_info.total_shares =
                decimal_summation_in_256(strategy_info.total_shares, user_shares);

            strategy_to_funds
                .entry(strategy_info.sic_contract_address.clone())
                .and_modify(|x| {
                    *x = x.checked_add(amount).unwrap();
                })
                .or_insert(amount);

            STRATEGY_MAP.save(deps.storage, U64Key::new(strategy_id), &strategy_info)?;
        }

        USER_REWARD_INFO_MAP.save(deps.storage, &user_addr, &user_reward_info)?;
    }

    strategy_to_funds.iter().for_each(|s2a| {
        let sic_address = s2a.0;
        let amount = s2a.1;

        messages.push(WasmMsg::Execute {
            contract_addr: String::from(sic_address),
            msg: to_binary(&sic_execute_msg::TransferRewards {}).unwrap(),
            funds: vec![Coin::new(amount.u128(), state.scc_denom.clone())],
        })
    });

    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.rewards_in_scc = state
            .rewards_in_scc
            .checked_add(total_rewards_in_scc)
            .unwrap();
        Ok(state)
    })?;

    Ok(Response::new()
        .add_messages(messages)
        .add_attribute("failed_strategies", failed_strategies.join(","))
        .add_attribute(
            "users_with_zero_deposits",
            users_with_zero_deposits.join(","),
        ))
}

// This assumes that the validator contract will transfer ownership of the airdrops
// from the validator contract to the SCC contract.
pub fn update_user_airdrops(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    update_user_airdrops_requests: Vec<UpdateUserAirdropsRequest>,
) -> Result<Response, ContractError> {
    // check for manager?
    let state = STATE.load(deps.storage)?;
    if info.sender != state.delegator_contract {
        return Err(ContractError::Unauthorized {});
    }

    if update_user_airdrops_requests.is_empty() {
        return Ok(Response::new().add_attribute("zero_user_airdrop_requests", "1"));
    }

    // iterate thru update_user_airdrops_request
    for user_request in update_user_airdrops_requests {
        let user = user_request.user;
        let user_airdrops = user_request.pool_airdrops;

        let config = CONFIG.load(deps.storage)?;
        // fetch the user rewards info
        let mut user_reward_info =
            if let Some(user_reward_info) = USER_REWARD_INFO_MAP.may_load(deps.storage, &user)? {
                user_reward_info
            } else {
                UserRewardInfo::new(config.default_user_portfolio)
            };

        user_reward_info.pending_airdrops = merge_coin_vector(
            &user_airdrops,
            CoinVecOp {
                fund: user_reward_info.pending_airdrops,
                operation: Operation::Add,
            },
        );

        USER_REWARD_INFO_MAP.save(deps.storage, &user, &user_reward_info)?;
    }

    Ok(Response::default())
}

#[cfg_attr(not(feature = "library"), entry_point)]
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetState {} => to_binary(&query_state(deps)?),
        QueryMsg::GetConfig {} => to_binary(&query_config(deps)?),
        QueryMsg::GetStrategyInfo { strategy_id } => {
            to_binary(&query_strategy_info(deps, strategy_id)?)
        }
        QueryMsg::GetUserRewardInfo { user } => to_binary(&query_user_reward_info(deps, user)?),
        QueryMsg::GetUndelegationBatchInfo {
            strategy_id,
            batch_id,
        } => to_binary(&query_undelegation_batch_info(deps, strategy_id, batch_id)?),
        QueryMsg::GetStrategiesList { start_after, limit } => {
            to_binary(&query_strategies_list(deps, start_after, limit)?)
        }
        QueryMsg::GetAllStrategies { start_after, limit } => {
            to_binary(&query_get_all_strategies(deps, start_after, limit)?)
        }
        QueryMsg::GetUser { user } => to_binary(&query_user(deps, user)?),
        QueryMsg::GetSharesPerTokenRatio { strategy_id } => {
            to_binary(&query_s_t_ratio(deps, strategy_id)?)
        }
    }
}

fn query_s_t_ratio(deps: Deps, strategy_id: u64) -> StdResult<GetSharesPerTokenRatioResponse> {
    if strategy_id.eq(&0) {
        return Err(StdError::GenericErr {
            msg: "retain_rewards has no shares per token ratio".to_string(),
        });
    }

    let strategy_info =
        if let Some(si) = STRATEGY_MAP.may_load(deps.storage, U64Key::from(strategy_id))? {
            si
        } else {
            return Err(StdError::GenericErr {
                msg: format!("Strategy not found {}", strategy_id),
            });
        };

    let shares_per_token_ratio = get_strategy_shares_per_token_ratio(deps.querier, &strategy_info)?;

    Ok(GetSharesPerTokenRatioResponse {
        shares_per_token_ratio: Some(shares_per_token_ratio),
    })
}

fn query_user(deps: Deps, user: String) -> StdResult<GetUserResponse> {
    let config = CONFIG.load(deps.storage)?;
    let user = deps.api.addr_validate(user.as_str())?;
    let user_reward_info_opt = USER_REWARD_INFO_MAP.may_load(deps.storage, &user)?;
    if user_reward_info_opt.is_none() {
        return Ok(GetUserResponse {
            user: Option::from(UserRewardInfoQuery {
                total_airdrops: vec![],
                retained_rewards: Uint128::zero(),
                undelegation_records: vec![],
                user_strategy_info: vec![],
                user_portfolio: config.default_user_portfolio,
            }),
        });
    }

    let user_reward_info = user_reward_info_opt.unwrap();

    let mut user_strategy_query: UserRewardInfoQuery = UserRewardInfoQuery {
        total_airdrops: vec![],
        retained_rewards: user_reward_info.pending_rewards,
        undelegation_records: user_reward_info.undelegation_records,
        user_strategy_info: vec![],
        user_portfolio: user_reward_info.user_portfolio,
    };

    let mut user_strategy_info: Vec<UserStrategyQueryInfo> = vec![];
    let mut total_airdrops: Vec<Coin> = user_reward_info.pending_airdrops;
    for user_strategy in user_reward_info.strategies {
        let strategy_id = user_strategy.strategy_id;

        let strategy_info = if let Some(strategy_info) =
            STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?
        {
            strategy_info
        } else {
            continue;
        };

        let user_shares = user_strategy.shares;
        let user_airdrop_pointer = &user_strategy.airdrop_pointer;

        // if we fail to fetch the tokens from the strategy, just default it to 10 for the query
        let strategy_s_t_ratio = if let Ok(strategy_s_t_ratio) =
            get_strategy_shares_per_token_ratio(deps.querier, &strategy_info)
        {
            strategy_s_t_ratio
        } else {
            return Err(StdError::GenericErr {
                msg: format!("sic failed to return result: {}", strategy_id),
            });
        };

        let user_rewards = get_staked_amount(strategy_s_t_ratio, user_shares);
        let user_airdrops = get_user_airdrops(
            &strategy_info.global_airdrop_pointer,
            user_airdrop_pointer,
            user_shares,
        )
        .unwrap_or_default();

        user_strategy_info.push(UserStrategyQueryInfo {
            strategy_id,
            strategy_name: strategy_info.name,
            total_rewards: user_rewards,
            total_airdrops: user_airdrops.clone(),
        });

        total_airdrops = merge_coin_vector(
            &user_airdrops,
            CoinVecOp {
                fund: total_airdrops,
                operation: Operation::Add,
            },
        );
    }
    user_strategy_query.total_airdrops = total_airdrops;
    user_strategy_query.user_strategy_info = user_strategy_info;

    Ok(GetUserResponse {
        user: Some(user_strategy_query),
    })
}

fn query_get_all_strategies(
    deps: Deps,
    start_after: Option<u64>,
    limit: Option<u32>,
) -> StdResult<GetAllStrategiesResponse> {
    let state = STATE.load(deps.storage)?;
    let limit = limit
        .unwrap_or(DEFAULT_PAGINATION_LIMIT)
        .min(MAX_PAGINATION_LIMIT) as u64;
    let start = if let Some(s) = start_after { s + 1 } else { 0 };

    let mut all_strategies_info: Vec<StrategyInfoQuery> = vec![];

    for i in start..std::cmp::min(state.next_strategy_id, start + limit) {
        let strategy_info =
            if let Some(strategy_info) = STRATEGY_MAP.may_load(deps.storage, U64Key::new(i))? {
                strategy_info
            } else {
                continue;
            };

        // this is retain rewards strategy
        if i.eq(&0) {
            all_strategies_info.push(StrategyInfoQuery {
                strategy_id: 0,
                strategy_name: strategy_info.name,
                total_rewards: state.rewards_in_scc,
                rewards_in_undelegation: Uint128::zero(),
                is_active: true,
                total_airdrops_accumulated: vec![],
                unbonding_period: 0,
                unbonding_buffer: 0,
                // special case
                sic_contract_address: strategy_info.sic_contract_address,
                undelegation_frequency: strategy_info.undelegation_cooldown,
            });
            continue;
        }

        let strategy_s_t_ratio = if let Ok(strategy_s_t_ratio) =
            get_strategy_shares_per_token_ratio(deps.querier, &strategy_info)
        {
            strategy_s_t_ratio
        } else {
            return Err(StdError::GenericErr {
                msg: format!("sic failed to return result: {}", i),
            });
        };

        let total_strategy_tokens =
            get_staked_amount(strategy_s_t_ratio, strategy_info.total_shares);
        let total_tokens_in_undelegation =
            get_staked_amount(strategy_s_t_ratio, strategy_info.current_undelegated_shares);

        all_strategies_info.push(StrategyInfoQuery {
            strategy_id: i,
            strategy_name: strategy_info.name,
            total_rewards: total_strategy_tokens,
            rewards_in_undelegation: total_tokens_in_undelegation,
            is_active: strategy_info.is_active,
            total_airdrops_accumulated: strategy_info.total_airdrops_accumulated,
            unbonding_period: strategy_info.unbonding_period,
            unbonding_buffer: strategy_info.unbonding_buffer,
            sic_contract_address: strategy_info.sic_contract_address,
            undelegation_frequency: strategy_info.undelegation_cooldown,
        });
    }

    Ok(GetAllStrategiesResponse {
        all_strategies: Some(all_strategies_info),
    })
}

fn query_undelegation_batch_info(
    deps: Deps,
    strategy_id: u64,
    batch_id: u64,
) -> StdResult<GetUndelegationBatchInfoResponse> {
    let undelegation_batch_info = UNDELEGATION_BATCH_MAP.may_load(
        deps.storage,
        (U64Key::new(batch_id), U64Key::new(strategy_id)),
    )?;
    Ok(GetUndelegationBatchInfoResponse {
        undelegation_batch_info,
    })
}

fn query_strategies_list(
    deps: Deps,
    start_after: Option<u64>,
    limit: Option<u32>,
) -> StdResult<GetStrategiesListResponse> {
    let state = STATE.load(deps.storage)?;
    let mut strategies_list: Vec<String> = vec![];
    let limit = limit
        .unwrap_or(DEFAULT_PAGINATION_LIMIT)
        .min(MAX_PAGINATION_LIMIT) as u64;
    let start = if let Some(s) = start_after { s + 1 } else { 0 };

    for i in start..std::cmp::min(state.next_strategy_id, start + limit) {
        let strategy_info =
            if let Some(strategy_info) = STRATEGY_MAP.may_load(deps.storage, U64Key::new(i))? {
                strategy_info
            } else {
                continue;
            };

        strategies_list.push(strategy_info.name);
    }

    Ok(GetStrategiesListResponse {
        strategies_list: Some(strategies_list),
    })
}

fn query_config(deps: Deps) -> StdResult<GetConfigResponse> {
    let config = CONFIG.load(deps.storage)?;
    Ok(GetConfigResponse {
        config: Some(config),
    })
}

fn query_user_reward_info(deps: Deps, user: String) -> StdResult<GetUserRewardInfo> {
    let user = deps.api.addr_validate(user.as_str())?;
    let user_reward_info = USER_REWARD_INFO_MAP.may_load(deps.storage, &user)?;
    Ok(GetUserRewardInfo { user_reward_info })
}

fn query_strategy_info(deps: Deps, strategy_id: u64) -> StdResult<GetStrategyInfoResponse> {
    let strategy_info = STRATEGY_MAP.may_load(deps.storage, U64Key::new(strategy_id))?;
    Ok(GetStrategyInfoResponse { strategy_info })
}

fn query_state(deps: Deps) -> StdResult<GetStateResponse> {
    let state = STATE.load(deps.storage)?;
    Ok(GetStateResponse {
        state: Option::from(state),
    })
}
